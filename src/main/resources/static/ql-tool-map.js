...
(function () {
    ...
    let mode = 'view';
    let dragging = false, dragTarget = null, panning = false;
    ...
    let onLayoutChanged = null, onSelectionChanged = null, onViewChanged = null;
    ...
    function setLayout(layoutData, options) {
        ...
        if (coordinateLayout) {
            layoutData.forEach((item, index) => {
                if (!item || item.code === undefined || item.code === null) return;
                nodes.push({
                    id: String(item.code),
    ...
    function drawLink(link, index) {
        ...
        drawRoute(route, CFG.railUnderlay, Math.max(8, 3.2 / transform.scale));
        drawRoute(route, color, Math.max(selected || hovered ? 3.5 : 2, (selected || hovered ? 2.4 : 1.45) / transform.scale));
        drawRouteArrow(route, color);
        if (selected) {
            drawRoute(route, 'rgba(255,255,255,.8)', .8);
            drawRouteArrow(route, CFG.selected);
        }
    }
    ...
    function drawRouteArrow(route, color) {
        ...
        if (!best || best.length < 8) return;
        const ratio = best.length > 52 ? 0.68 : 0.55;
        ...
        const size = Math.max(4.5 / transform.scale, Math.min(7, best.length * 0.3));
        ctx.save();
        ctx.fillStyle = color;
        ctx.beginPath();
    ...
    function drawNode(node) {
        ...
        ctx.save();
        ctx.shadowColor = state.alarm ? 'rgba(251,79,100,.62)' : (state.occupied ? 'rgba(16,185,129,.38)' : (selected || hovered ? 'rgba(34,211,238,.48)' : 'rgba(0,0,0,.35)'));
        ctx.shadowBlur = state.alarm ? 18 : (selected || hovered ? 14 : 7);
        ctx.shadowOffsetY = 2;
        ctx.fillStyle = colors.card;
        ctx.strokeStyle = selected ? CFG.selected : (hovered ? CFG.hover : colors.border);
        ctx.lineWidth = state.alarm || selected || hovered ? 2 : 1.2;
        ...
        ctx.fillRect(x + 4, y + 27, w - 4, h - 27);
        ctx.restore();
        ...
        ctx.font = `800 ${readableFontSize(13, 11.5, 23)}px ${CFG.fontFamily}`;
        ctx.fillText(ellipsis(node.code, 15), x + 10, y + 16);

        ctx.beginPath(); ctx.arc(x + w - 11, y + 16, 4, 0, Math.PI * 2);
        ctx.fillStyle = colors.rail; ctx.fill();
        if (state.alarm) {
            ctx.beginPath(); ctx.arc(x + w - 11, y + 16, 8, 0, Math.PI * 2);
            ctx.strokeStyle = 'rgba(251,79,100,.42)'; ctx.lineWidth = 3; ctx.stroke();
        }

        const halfBadgeWidth = (w - 20) / 2;
        drawNodeBadge(x + 7, y + 29, w - 14, 18,
            state.hasTask ? `任务 ${state.taskNo}` : '无任务',
            state.hasTask ? '#f59e0b' : '#64748b', state.hasTask, readableFontSize(9.5, 8.5, 17));
        drawNodeBadge(x + 7, y + 51, halfBadgeWidth, 15,
            state.occupied ? '● 占位' : '○ 空位',
            state.occupied ? '#10b981' : '#64748b', state.occupied, readableFontSize(8, 7, 14));
        drawNodeBadge(x + 13 + halfBadgeWidth, y + 51, halfBadgeWidth, 15,
            state.alarm ? '⚠ 报警' : '✓ 正常',
            state.alarm ? '#fb4f64' : '#22c55e', state.alarm, readableFontSize(8, 7, 14));
    }

    function drawOverviewNodeState(node, state, colors, x, y, w, h) {
        const invScale = 1 / Math.max(transform.scale, 0.01);
        const codeSize = Math.min(35, Math.max(18, 8.2 * invScale));
        const stateSize = Math.min(25, Math.max(13, 5.8 * invScale));
        ctx.save();
        ctx.textBaseline = 'middle';
        ctx.textAlign = 'left';
        ctx.fillStyle = colors.text;
        ctx.font = `850 ${codeSize}px ${CFG.fontFamily}`;
        ctx.fillText(ellipsis(node.code, 10), x + 7, y + 21);

        ctx.font = `800 ${stateSize}px ${CFG.fontFamily}`;
        ctx.fillStyle = state.occupied ? '#6ee7b7' : '#94a3b8';
        ctx.textAlign = 'left';
        ctx.fillText(state.occupied ? '占' : '空', x + 7, y + 53);

        ctx.fillStyle = state.hasTask ? '#fbbf24' : '#64748b';
        ctx.textAlign = 'center';
        ctx.fillText(state.hasTask ? `#${state.taskNo}` : '无任务', x + w / 2, y + 53);

        ctx.fillStyle = state.alarm ? '#fb7185' : '#4ade80';
        ctx.textAlign = 'right';
        ctx.fillText(state.alarm ? '警' : '常', x + w - 7, y + 53);
        ctx.restore();
    }
    ...
    function drawModeHint(width, height) {
        const labels = {
            view: '选择：点击轨道后可直接删除 · 滚轮缩放 · 拖拽空白区域平移',
    ...
    function onMouseMove(event) {
        ...
        canvas.style.cursor = node ? (mode === 'edit' ? 'move' : 'pointer') : (linkIndex >= 0 ? 'pointer' : (mode === 'view' ? 'grab' : 'default'));
    }
    ...
    function findNeighbor(point, direction, all) {
        ...
        const candidates = all.filter(item => {
            if (!item || item === point) return false;
    ...
    function getFullState(code) {
        ...
        if (!state) return { colorKey: 'unknown', statusLabel: '待匹配', occupied: false, alarm: false, hasTask: false, taskNo: '' };
        ...
        if (alarm) return { colorKey: 'alarm', statusLabel: state.alarmCodeLabel || '报警', occupied, alarm: true, hasTask, taskNo: state.taskNo };
        if (occupied) return { colorKey: 'occupied', statusLabel: '有货', occupied, alarm: false, hasTask, taskNo: state.taskNo || '' };
        if (hasTask) return { colorKey: 'task', statusLabel: '输送中', occupied, alarm: false, hasTask, taskNo: state.taskNo };
        if (offline) return { colorKey: 'offline', statusLabel: '离线', occupied, alarm: false, hasTask, taskNo: '' };
        return { colorKey: 'idle', statusLabel: state.pointStateLabel || '空闲', occupied, alarm: false, hasTask, taskNo: '' };
    }

    function getState(code) {
        const state = nodeStates[normCode(code)];
        if (!state) return { label: '待匹配', occupancyLabel: '-', occupied: false, taskNo: '', alarm: false, alarmLabel: '-' };
        return {
